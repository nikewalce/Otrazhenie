...
async def cmd_register(msg: Message):
    ...
    if not payload:
        await msg.answer('Чтобы зарегистрироваться отправьте: /register email | skin_type | goals | allergies\nПример: /register test@example.com | dry | увлажнение | нет')
        return
    ...
    with SessionLocal() as s:
        ...
        if not user:
            user = User(tg_id=msg.from_user.id, email=email, skin_type=skin_type, goals=goals, allergies=allergies)
            s.add(user)
            s.commit()
...
async def cmd_analyze(msg: Message):
    ...
    await msg.answer('Отправьте состав продукта текстом (через запятую), например: Water, Glycerin, Parfum...')
    ...
    with SessionLocal() as s:
        ...
        for a in analysis:
            if a.found:
                ...
                lines.append(f"{a.name} — {a.function or 'неизвестно'} — {a.safety}{allerg}")
            else:
                lines.append(f"{a.name} — ❓ Не найдено в базе")
        text = '\n'.join(lines)
        await msg.answer(text)
        # save product as temporary entry in products and user_products.history
        prod = Product(name=f'Анализ {datetime.utcnow().isoformat()[:19]}', composition_raw=composition_text)
        s.add(prod)
        s.commit()
...
async def cmd_addproduct(msg: Message):
    payload = msg.text.partition(' ')[2].strip()
    if not payload:
        await msg.answer('Чтобы добавить продукт: /addproduct Название | Бренд | состав (через запятую)')
        return
    parts = [p.strip() for p in payload.split('|')]
    if len(parts) < 3:
        await msg.answer('Неверный формат. Пример: /addproduct MyCream | BrandX | Water, Glycerin, ...')
        return
...
async def cmd_favorites(msg: Message):
    with SessionLocal() as s:
        ...
        for up in ups:
            ...
            texts.append(f'{up.custom_name or p.name} — добавлен {up.date_added.date()}')
        await msg.answer('\n'.join(texts))
...
async def cmd_history(msg: Message):
    with SessionLocal() as s:
        ...
        if not user:
            await msg.answer('Вы не зарегистрированы. Используйте /register')
            return
        ups = s.query(UserProduct).filter_by(user_id=user.id).order_by(UserProduct.date_added.desc()).limit(50).all()
        if not ups:
        ...
        for up in ups:
            p = s.query(Product).filter_by(id=up.product_id).first()
            out.append(f'{up.custom_name or p.name} — {up.date_added.strftime("%Y-%m-%d %H:%M")}')
        await msg.answer('\n'.join(out))
